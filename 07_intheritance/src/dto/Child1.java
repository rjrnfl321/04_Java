 ...
 * 		  자식이 자신의 코드인 것 처럼 사용
 * 상속 키워드 : extends (확장하다) 
 * 	 -> *부모* 코드를 물려받은 *자식* 의 크기가 커지기 때문에
 * 
 *  ** 주의사항 ** 
 *  - 부모의 코드 중 생성자는 물려받지 못함
 *  - 부모의 private 접근 제한자 필드/메서드는
 *    상속 받긴 하지만 직접 접근 불가
 *    -> 상속 관계여도 다른 클래스(객체)로 인식되기 때문에 접근불가 
 */
...
public class Child1 extends Parent {
	 ...
	 *  - super 뜻 : 상위의, 위쪽의
	 *  
	 *     -> 현재 클래스의 상위 클래스(부모) 호출하는 구문
	 *  - super()는 자식 생성에만 작성할 수 있음
	 *    반드시 첫 번째 줄에 작성해야함
	 * 
	 */
	...
	public void setNotbook(String notebook) {
		this.notebook = notebook; // this는 Child 객체를 의미
	}
