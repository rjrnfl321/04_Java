...
public class ArrayPractice { // *설계도

	// 필드
	Scanner sc = new Scanner(System.in); // *속성
	/**
	 * [실습 문제 1]
	...
	public void practice1() { // *기능
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if (i % 2 == 0) { // 짝수번째 인덱스 요소의 합
								// i를 2로 나눴을 때 0이 되는 요소
				sum += arr[i];
			}
		}
		System.out.println("\n짝수 번째 인덱스 합 : " + sum);
	 ...
	 * [실습 문제 2]
	 * 길이가 9인 배열을 선언 및 할당하고
	 * 9부터 1까지의 값을 반복문을 이용하여 순서대로
	 * 배열의 각 인덱스 요소에 대입하고 출력한 후
	...
	public void practice3() {
		...
		int num = sc.nextInt();
		int [] arr = new int[num]; // 입력 받은 크기만큼의 의 배열 할당
		for(int i=0; i<arr.length ; i ++) {
			arr[i] = i + 1; // 배열 요소에 값을 1부터 대입
			System.out.print(arr[i]+ " ");
		}
	}
	/*[실습 문제 4]
