...
public class BookService {
	...
	public List<BookDTO> selectWriter(String title) {
		
		List<BookDTO> SearchName = new ArrayList<BookDTO>();
		
		for(BookDTO book : bookList) {
			
			// 글쓴이가 포함되는 책을 searchList에 추가
			if(book.getWriter().contains(title)) {
				SearchName.add(book);
			}
		}
		return SearchName;
